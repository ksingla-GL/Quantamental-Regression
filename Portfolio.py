...
class Portfolio(object):
    ...
    """
    Creates a portfolio object which changes every rebalancing step according to the date,
    ...
    def compute_PnL(self,K,transaction_cost = 0.0005):
        ...
        for stock in pf_stocks:
            if stock in cur_holdings['Ticker'].values:
                ...
                if status=='Buy':
                    ...
                    stock,'Adj Close'].values[0]-1 - transaction_cost
                elif status=='Sell':
    ...
    def generate_trades_sheet(self,ts,pf_val,K):
        ...
        #Removing signals which are already present in our portfolio
        
        #####self.signals=self.signals[~self.signals['Ticker'].isin(self.old_holdings['Ticker'])]
        
        #Next I remove all rows from old holdings which contain the same stocks present
        #in our current signals too. Because we do not want to sell and buy stocks unnecessarily
        #and lose out on transaction costs.
        
        #####self.old_holdings=self.old_holdings[~self.old_holdings['Ticker'].isin(newsignals['Ticker'])]
        self.old_holdings['Shares']=(pf_val/K)/self.old_holdings['Adj Close']
        #self.old_holdings['Signal']=np.where(self.old_holdings['Signal']=="Buy","Sell","Buy")
        self.old_holdings['Exit_date']=self.date
        #if len(ts)>0:
            #ts=ts.append(self.signals[['Date','Ticker','Adj Close','Signal','Shares']])
